...
class GitClient {
  ...
  static Future<String> version() async {
    if (_version == null) {
      try {
        ExecutionResult rslt = await executeCommand('git', args:['--version'], writeToBuffer:true);
  ...
  Future<List<GitCommit>> listCommits({int limit, String range, String file, bool detail: false}) async {
    ...
    ExecutionResult result = await execute(args);
  ...
  Future<GitMergeResult> merge(String ref, {
    ...
    ExecutionResult proc = await execute(args);
  ...
  Future<GitMergeResult> cherryPick(String commit) async {
    ...
    ExecutionResult r = await execute(args);
    var result = new GitMergeResult();
  ...
  Future<ExecutionResult> execute(List<String> args, {
    bool binary: false,
...
class GitExitCodes {
  static const int STATUS_NOT_A_GIT_REPOSITORY = 128;
